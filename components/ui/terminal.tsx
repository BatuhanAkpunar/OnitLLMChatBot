...
/**
 * MagicUI Terminal (adapted): a MacOS-style window that types its children out
 * in sequence. Used for Build-mode run logs so progress feels alive. Source:
...
export const Terminal = ({
  ...
  const contextValue = useMemo<SequenceContextValue | null>(() => {
    ...
    return {
      completeItem: (index: number) =>
        setActiveIndex((cur) => (index === cur ? cur + 1 : cur)),
      activeIndex,
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(
        "scanlines relative z-0 w-full overflow-hidden rounded-xl border border-border bg-[#0c0c14] text-[#d6d6e6]",
        className,
      )}
    >
